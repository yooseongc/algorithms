...
public class BTreeSymbolTable<Key extends Comparable<Key>, Value> {

    private HashSet<PageSymbolTableInterface> pagesInMemory = new HashSet<>();
    private PageSymbolTableInterface<Key, Value> root = new BinarySearchSymbolTablePage<>(true, pagesInMemory);
    private int size;
    private Key sentinel;

    public BTreeSymbolTable(Key sentinel) {
        add(sentinel, null);
    ...
    private boolean contains(PageSymbolTableInterface<Key, Value> page, Key key) {
    ...
    private Value get(PageSymbolTableInterface<Key, Value> page, Key key) {
        if (page.isExternal()) return page.get(key);
        page.open();
    ...
    public void add(Key key, Value value) {
        ...
        if (root.isFull()) {
            PageSymbolTableInterface<Key, Value> leftHalf = root;
            PageSymbolTableInterface<Key, Value> rightHalf = root.split();
            root = new BinarySearchSymbolTablePage<>(false, pagesInMemory);
    ...
    private void add(PageSymbolTableInterface<Key, Value> page, Key key, Value value, boolean isNewKey) {
        if (isNewKey) page.incrementKeysSize();
        if (page.isExternal()) {
            page.add(key, value);
            return;
        }
        PageSymbolTableInterface<Key, Value> next = page.next(key);
    ...
    private Key min(PageSymbolTableInterface<Key, Value> page) {
        page.open();
        if (page.isExternal()) return page.min();
    ...
    private Key max(PageSymbolTableInterface<Key, Value> page) {
        page.open();
        if (page.isExternal()) return page.max();
    ...
    public Key floor(Key key) {
        PageSymbolTableInterface<Key, Value> pageThatMayContainKey = getPageThatMayContainKey(key);
        pageThatMayContainKey.open();
        Key floorKey = pageThatMayContainKey.floor(key);
        pageThatMayContainKey.close(false);
        if (floorKey == sentinel) floorKey = null;
        return floorKey;
    }
    ...
    public Key ceiling(Key key) {
        ...
        if (ceilingKey == null) {
            int rank = rank(key);
            if (rank < size()) ceilingKey = select(rank);
        }
        if (ceilingKey == sentinel) ceilingKey = min();
        return ceilingKey;
    }

    private PageSymbolTableInterface<Key, Value> getPageThatMayContainKey(Key key) {
        PageSymbolTableInterface<Key, Value> parent = getParentPageOfPageWithKey(key);
        if (parent == null) return root;
        return parent.next(key);
    }

    private PageSymbolTableInterface<Key, Value> getParentPageOfPageWithKey(Key key) {
        if (root.isExternal()) return null;
        return getParentPageOfPageWithKey(root, key);
    }

    private PageSymbolTableInterface<Key, Value> getParentPageOfPageWithKey(PageSymbolTableInterface<Key, Value> page, Key key) {
        PageSymbolTableInterface<Key, Value> nextPage = page.next(key);
        nextPage.open();
    ...
    public Key select(int index) {
        if (index < 0 || index >= size()) throw new IllegalArgumentException("Index cannot be negative and must be lower than BTree size");
        return select(root, index);
    }

    private Key select(PageSymbolTableInterface<Key, Value> page, int index) {
        if (page.isExternal()) return page.select(index);
        int leftSubtreeSize = 0;
        for (Key key : page.keys()) {
            PageSymbolTableInterface<Key, Value> childPage = page.getChildPage(key);
            childPage.open();
    ...
    private int rank(PageSymbolTableInterface<Key, Value> page, Key searchKey) {
        if (page.isExternal()) return page.rank(searchKey);
        PageSymbolTableInterface<Key, Value> nextPage = page.next(searchKey);
        int leftSubtreeSize = 0;
    ...
    private void delete(PageSymbolTableInterface<Key, Value> page, Key key, int treeLevel) {
        if (page.isExternal()) {
            page.delete(key);
        } else {
            ...
            for (Key KeyInInternalPage : page.keys()) {
                PageSymbolTableInterface<Key, Value> childPage = page.getChildPage(KeyInInternalPage);
    ...
    private boolean stabilizePages(PageSymbolTableInterface<Key, Value> page, PageSymbolTableInterface<Key, Value> parentPage, int treeLevel, Key keyBeingDeleted) {
        boolean pagesStable = false;
        boolean hasLeftSibling = false;
        boolean hasRightSibling = false;
        boolean wasMergeRequired = false;

        PageSymbolTableInterface<Key, Value> leftPageParent = null;
        PageSymbolTableInterface<Key, Value> leftPage = null;
        PageSymbolTableInterface<Key, Value> rightPageParent = null;
        PageSymbolTableInterface<Key, Value> rightPage = null;
        Key rightPageMinKey;
            ...
            if (minKeyRank > 0) {
                ...
                if (leftPage.pageSize() > page.maxNumberOfNodes() / 2) {
                    if (leftPage.isExternal()) {
                        ...
                        updateParentsSizesAndReference(leftPage, false, null, -1);
                        ...
                        PageSymbolTableInterface<Key, Value> leftPageChild = leftPage.next(leftPageMaxKey);
                        leftPage.deleteMax();
                        int leftPageChildKeysSize = leftPageChild.keysSize(true);
                        leftPage.setKeysSize(leftPage.keysSize(true) - leftPageChildKeysSize);
                        updateParentsSizesAndReference(leftPage, false, null, -leftPageChildKeysSize);
            ...
            if (maxKeyRank < size() - 1) {
                ...
                if (rightPage.pageSize() > page.maxNumberOfNodes() / 2) {
                    if (rightPage.isExternal()) {
                        ...
                        updateParentsSizesAndReference(rightPage, true, rightPageMinKey, -1);
                        ...
                        PageSymbolTableInterface<Key, Value> rightPageChild = rightPage.next(rightPageMinKey);
                        rightPage.deleteMin();
                        ...
                        updateParentsSizesAndReference(rightPage, true, rightPageMinKey, -rightPageChildKeysSize);
        ...
        // Close pages
        if (leftPageParent != null) leftPageParent.close(false);
        if (leftPage != null) leftPage.close(false);
        if (rightPageParent != null) rightPageParent.close(false);
        if (rightPage != null) rightPage.close(false);

        return wasMergeRequired;
    }

    private void merge(
            PageSymbolTableInterface<Key, Value> page, PageSymbolTableInterface<Key, Value> parentPage,
            PageSymbolTableInterface<Key, Value> siblingPage, boolean updateReference, Key keyToDelete) {
        ...
        // Page parents sizes and references will be updated while returning from the recursive delete method
        ...
        updateParentsSizesAndReference(siblingPage, updateReference, minKeyInSiblingParentPage, sizeToUpdateInSiblingParentPages);
        if (page.containsSentinel()) {
    ...
    private void updateParentPageReference(PageSymbolTableInterface<Key, Value> parentPage, PageSymbolTableInterface<Key, Value> page, Key currentMinKey) {
        parentPage.deletePageWithKey(currentMinKey);
        parentPage.addPage(page);
        parentPage.setKeysSize(parentPage.keysSize(true) + page.keysSize(true));
    }

    private void updateParentsSizesAndReference(PageSymbolTableInterface<Key, Value> page, boolean updateReference, Key keyOfPagesToDelete, int sizeToUpdate) {
        PageSymbolTableInterface<Key, Value> currentPage = page;
        PageSymbolTableInterface<Key, Value> currentParent = page.getParentPage();
        while (currentParent != null) {
            int currentParentKeysSize = currentParent.keysSize(true);
            int updatedKeysSize = currentParentKeysSize + sizeToUpdate;
            if (updateReference && currentParent.contains(keyOfPagesToDelete)) {
                currentParent.deletePageWithKey(keyOfPagesToDelete);
    ...
    private PageSymbolTableInterface<Key, Value> getPageWithKeyInTreeLevel(Key key, int treeLevel) {
        if (treeLevel < 0) throw new IllegalArgumentException("Tree level cannot be negative");
        return getPageWithKeyInTreeLevel(root, key, treeLevel, 0);
    }

    private PageSymbolTableInterface<Key, Value> getPageWithKeyInTreeLevel(PageSymbolTableInterface<Key, Value> page, Key key, int treeLevel, int currentTreeLevel) {
        page.open();
        if (currentTreeLevel == treeLevel) return page;
        if (page.isExternal()) throw new IllegalArgumentException("Tree only has " + currentTreeLevel + "levels.");
        PageSymbolTableInterface<Key, Value> nextPage = page.next(key);
        page.close(false);
        PageSymbolTableInterface<Key, Value> targetPage = getPageWithKeyInTreeLevel(nextPage, key, treeLevel, currentTreeLevel + 1);
        nextPage.close(false);
        return targetPage;
    }
    ...
    private void keys(PageSymbolTableInterface<Key, Value> page, Queue<Key> keys, Key low, Key high) {
        page.open();
        if (page.isExternal()) {
            for (Key key : page.keys()) {
                if (key != sentinel && low.compareTo(key) <= 0 && key.compareTo(high) <= 0) {
                    keys.enqueue(key);
                }
            }
            return;
        ...
        for (Key key : page.keys()) {
            PageSymbolTableInterface<Key, Value> nextPage = page.getChildPage(key);
            nextPage.open();
    ...
    public Iterable<Value> get(Key low, Key high) {
        if (low == null) throw new IllegalArgumentException("First argument to get() cannot be null");
        if (high == null) throw new IllegalArgumentException("Second argument to get() cannot be null");
        Queue<Value> values = new Queue<>();
        get(root, values, low, high);
        return values;
    }

    private void get(PageSymbolTableInterface<Key, Value> page, Queue<Value> values, Key low, Key high) {
        page.open();
        if (page.isExternal()) {
            for (Key key : page.keys()) {
                if (key != sentinel && low.compareTo(key) <= 0 && key.compareTo(high) <= 0) {
